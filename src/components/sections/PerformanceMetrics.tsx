...
import { Cpu, MemoryStick, Timer, ShieldCheck, AlertTriangle, Activity } from 'lucide-react';
    ...
    <div className="rounded-xl border border-border bg-surface p-6 card-hover animate-slide-up flex flex-col h-full">
      ...
      <div className="grid grid-cols-2 gap-3 flex-1 content-start">
        {data.map((metric) => {
          ...
          return (
            <div key={metric.id} className={cn('flex flex-col gap-2 p-3 rounded-lg border', statusClass)}>
              ...
              <div>
                ...
                <span className="text-[10px] text-current/60 ml-1">{metric.unit}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-current/70">{metric.label}</span>
                {metric.change !== 0 && (
                  <span className="text-[10px] font-mono">{formatPercent(metric.change)}</span>
                )}
              </div>
            </div>
          );
