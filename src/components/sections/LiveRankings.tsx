...
import { RankingsSkeleton } from "@/components/ui/CardSkeleton";
    ...
    <div className="rounded-xl border border-border bg-surface p-6 card-hover animate-slide-up flex flex-col h-full">
      ...
      <div className="space-y-1">
        {data.map((entry, i) => (
          ...
          >
            <span
              className={`text-xs font-bold font-mono ${entry.rank <= 3 ? "text-accent" : "text-muted"}`}
            >
              #{entry.rank}
            </span>
            ...
            <div className="flex items-center justify-end gap-0.5">
              ...
              <span
                className={`text-[10px] font-mono ${entry.trend === "up" ? "text-success" : entry.trend === "down" ? "text-error" : "text-muted"}`}
              >
